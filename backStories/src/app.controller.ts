...
import { CreateVideoDto, InfoDto } from './dto/CreateVideoDto';
...
const imageFileFilter = (req, file, callback) => {
  if (!parse(file.originalname).ext.toLowerCase().match(/\.(jpg|jpeg|png)$/)) {
    return callback(new Error('Only image files are allowed! ['+parse(file.originalname).ext+'] (x)' ), false);
  }
  callback(null, true);
};
...
export class AppController {
  ...
  async uploadMultipleFiles(@UploadedFiles() files,@Body() infos:any) {
    ...
    files.forEach(file => {
      ...
      if(transition.filter(elm=>elm.name == file.originalname).length > 0){
        tr  =true;
      }
      images.push( {
        path:file.path,
        ...
        disableFadeOut: true,
        filters : "zoompan=z='if(lte(zoom,1.0),1.5,max(1.001,zoom-0.0015))':d=125"
      })
    });

    console.log(images)
    await this.createVideo(images,project_dir+'/assets/videos/test1.mp4').then((elm)=>{
